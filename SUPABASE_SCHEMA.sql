...
-- FUNCTION: ensure_official_permission (TRIGGER)
-- Prevents non-admins from setting is_official = true
...
CREATE OR REPLACE FUNCTION public.increment_dislikes(doc_id BIGINT)
RETURNS VOID AS $$
BEGIN
  UPDATE public.documents
...
CREATE OR REPLACE FUNCTION public.decrement_dislikes(doc_id BIGINT)
RETURNS VOID AS $$
BEGIN
  UPDATE public.documents
...
CREATE OR REPLACE FUNCTION public.increment_bookmarks(doc_id BIGINT)
RETURNS VOID AS $$
BEGIN
  -- Note: documents table currently doesn't have a bookmark_count column in SQL,
  -- but it's often used for analytics. We keep these here for parity.
  NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Interactions: Private write
DROP POLICY IF EXISTS "Users can manage their own interactions" ON public.interactions;
CREATE POLICY "Users can manage their own interactions" ON public.interactions FOR ALL USING (auth.uid() = user_id);

-- Bookmarks: Private write
